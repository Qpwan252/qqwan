...
extern "C" fn handle() {
    ...
    match &action {
        SessionAction::StartGame => {
            ...
            debug!("handle: StartGame, status is {:x?}", session.status);
                ...
                SessionStatus::StartGameWaiting | SessionStatus::CheckWordWaiting => {
                    let msg_id: gstd::MessageId =
                        msg::send(state.wordle_program, Action::StartGame { user }, 0)
                            .expect("handle: error in sending `Action::StartGame`");
                ...
                SessionStatus::ReplyReceived(recv_event) => {
                    if let SessionEvent::GameStarted = recv_event {
                        ...
                        msg::reply(SessionEvent::GameStarted, 0)
                            .expect("handle: error in sending `GameStarted` reply");
                        debug!(
                        ...
                        .expect("handle: error in sending `SessionAction::CheckGameStatus`");
                    } else {
                        panic!(
                            "handle: StartGame, invalid received event: {:x?}",
                            recv_event
                        );
                    }
                }
                _ => panic!("handle: StartGame, wrong status, {:x?}", session.status),
            }
        }
        SessionAction::CheckWord { word } => {
            if !state.user_to_session.contains_key(&user) {
                panic!("handle: CheckWord, non-existing user");
            }

            let session: &mut Session = state.user_to_session.get_mut(&user).unwrap();
            debug!("handle: CheckWord, status is {:x?}", session.status);

            match &session.status {
                SessionStatus::CheckWordWaiting => {
                    if word.len() != 5 || !word.chars().all(|c| c.is_lowercase()) {
                        panic!("handle: CheckWord, invalid word, {}", word);
                    }
                        ...
                        || exec::block_height() > session.start_block + CHECK_GAME_STATUS_DELAY
                    {
                        ...
                        .expect("handle: CheckWord, error in replying `GameOver(Lose)`");
                    } else {
                        let msg_id = msg::send(state.wordle_program, Action::CheckWord { user, word }, 0)
                            .expect("handle: error in sending `Action::CheckWord`");
                        session.msg_ids = (msg_id, msg::id());
                ...
                SessionStatus::ReplyReceived(recv_event) => {
                    ...
                    } = recv_event
                    {
                        ...
                        if correct_positions.len() == 5 {
                            ...
                            .expect("handle: error in replying `GameOver(Win)`");
                            ...
                            .expect("handle: error in replying `GameOver(Lose)`");
                        } else {
                            ...
                            session.result = SessionResult::Ongoing;
                            msg::reply(event, 0)
                                .expect("handle: error in replying `WordChecked`");
                        }
                    } else {
                        panic!(
                            "handle: CheckWord, invalid ReplyReceived event: {:x?}",
                            recv_event
                        );
                    }
                }
                _ => panic!("handle: CheckWord, wrong status ({:x?})", session.status),
            }
        }
        SessionAction::CheckGameStatus { user } => {
            debug!("handle: CheckGameStatus");
            if !state.user_to_session.contains_key(user) {
                panic!("handle: CheckGameStatus, non-existing user");
            }

            let session: &mut Session = state.user_to_session.get_mut(user).unwrap();
            debug!(
                "handle: CheckGameStatus, block_height={}, start_block={}",
                exec::block_height(),
                session.start_block
            );
                ...
                && session.result == SessionResult::Ongoing
            {
                ...
                .expect("handle: CheckGameStatus, error in sending `GameOver(Lose)`");
            }
        }
    }
}
